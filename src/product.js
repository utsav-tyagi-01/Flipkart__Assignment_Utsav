...
searchInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter" && searchInput.value.trim() !== "") {
        console.log(`product.html?query=${encodeURIComponent(searchInput.value.trim())}`);
...
function applyFilters() {
    ...
    const filteredProducts = currentProducts.filter(product => {
      const categoryMatch = selectedFilters.category.length === 0 || selectedFilters.category.includes(product.category.toLowerCase());
      const brandMatch = selectedFilters.brand.length === 0 || selectedFilters.brand.includes(product.brand?.toLowerCase());
      const ratingMatch = selectedFilters.rating.length === 0 || selectedFilters.rating.some(r => product.rating >= parseInt(r));
      const colorMatch = selectedFilters.color.length === 0 || selectedFilters.color.includes(product.color?.toLowerCase());
      const discountMatch = selectedFilters.discount.length === 0 || selectedFilters.discount.some(d => product.discountPercentage >= parseInt(d));
      const availabilityMatch = selectedFilters.availability || product.stock > 0;
      const priceMatch = product.price <= parseInt(maxPrice);
  
      return categoryMatch && brandMatch && ratingMatch && colorMatch && discountMatch && availabilityMatch && priceMatch;
    });


    filtersApplied = Object.values(selectedFilters).some(filterArray => filterArray.length > 0) || maxPrice < 5000;
    toggleClearAllButton();

    const searchInputValue = document.querySelector(".header__search-input")?.value.trim();
    const urlParams = new URLSearchParams(window.location.search);
    ...
    if (resultsCountElem) {
      resultsCountElem.innerHTML =
        `Showing 1 - ${totalCount} of ${totalCount} results for "<span>${keyword}</span>"`;
    }
...
function capitalizeFirstLetter(str) {
  ...
  function updateResultsHeader(query, totalCount) {
    document.getElementById("categoryName").textContent = capitalizeFirstLetter(query);
    document.getElementById("searchTerm").textContent = query;
    document.getElementById("resultsCount").innerHTML = `Showing 1 – ${totalCount} of ${totalCount} results for "<span>${query}</span>"`;
  }

  document.querySelectorAll(".sort__btn").forEach(btn => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".sort__btn").forEach(b => b.classList.remove("active"));
